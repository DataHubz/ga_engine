use crate::{vector::Vec3, ga::geometric_product_full, bivector::Bivector3};
...
pub fn reflect_ga(v: Vec3, b: Bivector3) -> Vec3 {
    let mut v_mv  = [0.0;8];
         v_mv[1] = v.x; v_mv[2] = v.y; v_mv[3] = v.z;
    let mut b_mv  = [0.0;8]; b_mv[4]=b.xy; b_mv[5]=b.yz; b_mv[6]=b.zx;
    let mut tmp   = [0.0;8];
    let mut out   = [0.0;8];
    // b * v
    geometric_product_full(&b_mv, &v_mv, &mut tmp);
    // (b * v) * (–b)
    let mut b_neg = b_mv; b_neg[4..7].iter_mut().for_each(|x| *x = -*x);
